...
import { ALI_ILE_ELA_DRAFT } from "../../src/content/drafts/2026-08-27-ali-ile-ela.js";

const story = ALI_ILE_ELA_DRAFT;
...
test("Ali ile Ela: yaş, gerçek süre ve anlamlı bölüm sözleşmesi", () => {
...
test("Ali ile Ela: cümle, paragraf, sözlük ve anlatı sözleşmesi", () => {
...
test("Ali ile Ela: özgünlük izi açık, insan onayı olmadan release kapalı", () => {
...
test("Tam katalog audit'i 14-16 ve 16-18 yaşlarını doğrudan eşler", async () => {
  const source = await import("node:fs").then(({ readFileSync }) =>
    readFileSync(new URL("../../scripts/audit-full-catalog.mjs", import.meta.url), "utf8"),
  );

  assert.match(source, /"14-16":\s*"14-16"/u);
