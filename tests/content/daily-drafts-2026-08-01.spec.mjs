...
import { OKI_VE_AY_HARITASI_DRAFT } from "../../src/content/drafts/2026-08-01-oki-ve-ay-haritasi.js";

const story = OKI_VE_AY_HARITASI_DRAFT;
...
test("Oki ve Ay Haritası: yaş, süre ve bölüm sözleşmesi", () => {
...
test("Oki ve Ay Haritası: dil, paragraf, sözlük ve tema sözleşmesi", () => {
...
test("Oki ve Ay Haritası: factual düzeltme ve insan onayı olmadan yayımlanamaz", () => {
  const review = story.contentQualityReview;
  assert.match(body, /rüzgâr aşınması gerçekleşmiyordu/u);
  assert.match(body, /akan sıvı su aşınması yoktu/u);
  assert.match(body, /su buzu/u);
  assert.match(body, /su molekülleri/u);
