...
import { NANA_ANLATIYOR_DRAFT } from "../../src/content/drafts/2026-08-18-nana-anlatiyor.js";
import { evaluateContentQualityReview } from "../../src/content/contentQualityReview.js";

const story = NANA_ANLATIYOR_DRAFT;
...
test("Nana Anlatıyor yaş, gerçek süre ve bölüm sözleşmesini karşılar", () => {
...
test("Nana Anlatıyor manifesto dili ve tamamlanmış anlatı yapısını karşılar", () => {
  ...
  assert.match(body, /son resim hâlâ ortada yoktu/u);
  assert.match(body, /lale kartı sıraya uyuyordu/u);
  assert.match(body, /ortak öyküsü olmuştu/u);
});

test("Nana Anlatıyor insan onayı olmadan yalnız yapısal adaydır", () => {
