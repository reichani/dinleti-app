...
s = s.replace(/<div data-okuma-modlari[\s\S]*?<\/div>\n\s*<div data-okuma-modu-ipucu[\s\S]*?<\/div>/, `<div data-okuma-modu-kompakt style={{ display: "flex", justifyContent: "center", marginTop: 7 }}>
            ...
            <div data-okuma-modu-ipucu style={{ margin: "5px auto 0", maxWidth: 390, color: S.soluk, fontSize: 11, lineHeight: 1.35, textAlign: "center" }}>{okumaModuAyar.aciklama}</div>`);

s = s.replace('>Yardım · Oku</button>', '>? Yardım</button>');
s = s.replace(/<div style=\{\{ display: "flex", justifyContent: "center", gap: 10, marginTop: 8, flexWrap: "wrap" \}\}>[\s\S]*?<\/div>/, `<div data-alt-araclar style={{ display: "flex", justifyContent: "center", gap: 4, marginTop: 6 }}>
...
s = s.replace('style={{ width: "min(480px, 100%)", background: `linear-gradient(180deg, ${aktif.renk[0]}55 0%, ${S.fon} 30%)`, backgroundColor: S.fon, display: "flex", flexDirection: "column", height: "100dvh", maxHeight: "100dvh", overflow: "hidden", padding:', 'style={{ width: "min(480px, 100%)", background: `linear-gradient(180deg, ${aktif.renk[0]}55 0%, ${S.fon} 30%)`, backgroundColor: S.fon, display: "flex", flexDirection: "column", height: "100dvh", maxHeight: "100dvh", overflow: "hidden", padding:');
